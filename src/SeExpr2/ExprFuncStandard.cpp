...
ExprType ExprFuncStandard::prep(ExprFuncNode* node, bool scalarWanted, ExprVarEnvBuilder& envBuilder) const
{
    if (_funcType < VEC) {
        ...
        for (int c = 0; c < node->numChildren(); c++) {
            ...
            if (childDim != 1) {
                if (nonOneDim != 1 && childDim != nonOneDim)
                    multiInvoke = false;
                nonOneDim = childDim;
            }
        }
...
int Func2VVOp(const int* opData, double* fp, char** c)
{
...
int FuncNVOp(const int* opData, double* fp, char** c)
{
    int n = opData[1];
    Vec3d* vals = static_cast<Vec3d*>(alloca(n * sizeof(Vec3d)));
    for (int k = 0; k < n; k++)
        new (vals + k) Vec3d(Vec3dRef(&fp[opData[k + 2]]));  // placement new!
    double* out = &fp[opData[n + 2]];
    *out = ((ExprFuncStandard::Funcnv*)(c[opData[0]]))(n, vals);
    return 1;
}
int FuncNVVOp(const int* opData, double* fp, char** c)
{
    int n = opData[1];
    Vec3d* vals = static_cast<Vec3d*>(alloca(n * sizeof(Vec3d)));
    for (int k = 0; k < n; k++)
